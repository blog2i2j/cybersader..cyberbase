...
var require_lib = __commonJS({
  "node_modules/obsidian-dataview/lib/index.js"(exports) {
    ...
    (function(module3, exports2) {
      !function(n2, t) {
        ...
        return function(n2) {
          ...
          e.all = on2, e.alt = T, e.any = un, e.cr = dn, e.createLanguage = function(n3) {
            var t2 = {};
            for (var r2 in n3)
    ...
    var QUERY_LANGUAGE = parsimmon_umd_minExports.createLanguage({
      ...
      sortByClause: (q) => parsimmon_umd_minExports.seqMap(parsimmon_umd_minExports.regexp(/SORT/i), parsimmon_umd_minExports.whitespace, q.sortField.sepBy1(parsimmon_umd_minExports.string(",").trim(parsimmon_umd_minExports.optWhitespace)), (sort2, _1, fields) => {
...
// src/modals/slot-modal.ts
var import_obsidian6 = require("obsidian");

// src/utils/render-utils.ts
var import_obsidian = require("obsidian");
async function renderSlot(plugin, settings, slot) {
  var _a;
  const app = plugin.app;
  slot.addClass("tier-list-slot");
  const link = slot.find("a.internal-link");
  if (link && !((_a = link.getAttribute("href")) == null ? void 0 : _a.match(/\.(jpeg|jpg|gif|png|webp)$/i))) {
  ...
  setTimeout(() => {
    slot.findAll(".excalidraw-embedded-img").forEach((excalidrawEl) => {
      ...
      (_a2 = excalidrawEl.parentElement) == null ? void 0 : _a2.replaceChild(newElement, excalidrawEl);
    });
  }, 50);
...
async function moveLinesInActiveFile(app, startIndex, count, newIndex2, correction = true) {
  const file = app.workspace.getActiveFile();
  if (!file || startIndex == newIndex2) {
    return;
  }
  await app.vault.process(file, (content) => {
    let lines = content.split("\n");
    const removedLines = lines.splice(startIndex, count);
    if (newIndex2 > startIndex && correction) {
      newIndex2 -= count;
    }
    lines.splice(newIndex2, 0, ...removedLines);
    return lines.join("\n");
  });
}
async function replaceLineInActiveFile(app, lineNumber, newText) {
  const file = app.workspace.getActiveFile();
  if (!file)
    return;
  await app.vault.process(file, (content) => {
...
async function replaceLinesInActiveFile(app, startLine, lineCount, newLines) {
  const file = app.workspace.getActiveFile();
  if (!file)
    return;
  await app.vault.process(file, (content) => {
...
async function readLineFromActiveFile(app, lineNumber) {
...
async function deleteLineInActiveFile(app, lineNumber) {
  const file = app.workspace.getActiveFile();
  if (!file)
    return;
  await app.vault.process(file, (content) => {
...
async function insertLineInActiveFile(app, lineNumber, newText) {
  const file = app.workspace.getActiveFile();
  if (!file)
    return;
  await app.vault.process(file, (content) => {
    const lines = content.split("\n");
    const index2 = Math.max(0, Math.min(lineNumber, lines.length));
    lines.splice(index2, 0, newText);
    return lines.join("\n");
  });
}

// src/modals/request-modal.ts
var import_obsidian2 = require("obsidian");
var DataviewSearchModal = class extends import_obsidian2.Modal {
  constructor(app, from, where, onApply) {
    super(app);
    this.from = "";
    this.where = "";
    this.foundFiles = [];
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("From").addText((text) => {
      text.setValue(this.from);
      text.onChange((value) => this.from = value);
    });
    new import_obsidian2.Setting(contentEl).setName("Where").addText((text) => {
      text.setValue(this.where);
      text.onChange((value) => this.where = value);
    });
    new import_obsidian2.Setting(this.contentEl).setName("Count").addText((text) => {
      this.countComponent = text;
      text.setValue("0");
      text.inputEl.readOnly = true;
    });
    new import_obsidian2.Setting(this.contentEl).setName("Files").setClass("tier-list-query").addTextArea((textArea) => {
    ...
    new import_obsidian2.Setting(contentEl).addButton(
      (btn) => btn.setIcon("refresh-ccw").setCta().onClick(() => this.updateFiles())
    ).addButton(
...
async function searchFiles(from, where) {
  ...
  try {
    ...
    return result.value.values.map((p) => dv.page(p.path).file.name);
  } catch (error) {
    console.log(error.message);
    return [];
  }
}
function generateTierListPostProcessor(plugin) {
  const app = plugin.app;
  let scroll = 0;
  return async (tierList, ctx) => {
    const tagEl = tierList.find(`a[href="${plugin.settings.tag}"]`);
    if (!tagEl)
      return;
    tagEl.remove();
      ...
      if (settingsList) {
        for (const setting of settingsList.findAll("li")) {
          ...
          if (fileKey.toLowerCase() == key.toLowerCase()) {
            const settingLine2 = findDataLine(setting);
            scroll = scrollableEl.scrollTop;
            await replaceLineInActiveFile(app, settingLine2, valueText);
            return;
          }
        }
    ...
    async function writeSettings(settings) {
      ...
      if (settingsList) {
        ...
        const settingsLine = ((_a = ctx.getSectionInfo(tierList)) == null ? void 0 : _a.lineEnd) || 0;
        scroll = scrollableEl.scrollTop;
    ...
    async function addMissingSlots(names, line) {
    ...
    function addListContextMenuOptions(menu, line) {
      ...
      menu.addItem((item) => item.setTitle("Settings").setIcon("settings").onClick(() => {
        new LocalSettingsModal(app, localSettings, (updatedSettings) => {
          scroll = scrollableEl.scrollTop;
    ...
    async function addSlotContextMenuOptions(menu, line) {
      const str = await readLineFromActiveFile(app, line);
    ...
    async function initializeSlots() {
      for (const list of tierList.findAll("ul > li > ul")) {
        sortable_esm_default.create(list, {
          group: `slot-${sectionInfo == null ? void 0 : sectionInfo.lineStart}`,
          animation: localSettings.animation,
          fallbackOnBody: true,
          forceFallback: true,
          onEnd: async (evt) => {
            var _a, _b, _c, _d, _e, _f, _g;
            const tierListLine = parseInt(((_d = (_c = (_b = (_a = evt.item.parentElement) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.parentElement) == null ? void 0 : _c.parentElement) == null ? void 0 : _d.getAttr("data-line")) || "0");
            const parentLine = parseInt(((_f = (_e = evt.item.parentElement) == null ? void 0 : _e.parentElement) == null ? void 0 : _f.getAttr("data-line")) || "0", 10);
            const newIndex2 = evt.newIndex || 0;
            const oldParentIndex = parseInt(((_g = evt.from.parentElement) == null ? void 0 : _g.getAttr("data-line")) || "0");
            const oldIndex2 = evt.oldIndex || 0;
            let newLine = tierListLine + parentLine + newIndex2 + 1;
            let oldLine = tierListLine + oldParentIndex + oldIndex2 + 1;
    ...
    function initializeRows() {
      sortable_esm_default.create(tierList.find(":scope > ul"), {
        handle: ".tier-list-tier",
        group: `tier-${sectionInfo == null ? void 0 : sectionInfo.lineStart}`,
        animation: localSettings.animation,
        onEnd: async (evt) => {
          var _a, _b, _c;
          if (evt.oldIndex == evt.newIndex)
            return;
          const tier = evt.item;
          const tierLine = parseInt(((_b = (_a = tier.parentElement) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.getAttr("data-line")) || "0");
          const oldLine = parseInt(tier.getAttr("data-line") || "0") + tierLine;
          let newIndex2 = evt.newIndex || 0;
          let oldIndex2 = evt.oldIndex || 0;
          const ul = tier.parentElement;
          const length = tier.find("ul").children.length + 1;
          const oldChild = ul == null ? void 0 : ul.children[newIndex2 + (newIndex2 > oldIndex2 ? -1 : 1)];
          const oldChildLength = (((_c = oldChild == null ? void 0 : oldChild.find("ul")) == null ? void 0 : _c.children.length) || 0) + 1;
          const oldChildLine = parseInt((oldChild == null ? void 0 : oldChild.getAttr("data-line")) || "0");
    ...
    async function filterTierListNames(names) {
      if (!sectionInfo)
        return names;
      const activeFile = app.workspace.getActiveFile();
      if (!activeFile)
...
var FileInputSuggest = class extends import_obsidian5.AbstractInputSuggest {
...
var InputType = /* @__PURE__ */ ((InputType2) => {
  ...
  InputType2["InternalEmbed"] = "Internal Embed";
  InputType2["InternalLink"] = "Internal Link / Cover";
  InputType2["ExternalEmbed"] = "External Embed";
  InputType2["ExternalLink"] = "External Link";
  return InputType2;
})(InputType || {});
var ParsedInput = class {
  constructor(rawValue) {
    this.defaultColor = getComputedStyle(document.body).getPropertyValue("--background-secondary");
  ...
  toString() {
    let output = "";
    switch (this.type) {
      case "Internal Embed" /* InternalEmbed */:
        output = `![[${this.value}${this.alias ? ` | ${this.alias}` : ""}]]`;
        break;
      case "Internal Link / Cover" /* InternalLink */:
        output = `[[${this.value}${this.alias ? ` | ${this.alias}` : ""}]]`;
        break;
      case "External Embed" /* ExternalEmbed */:
        output = `![${this.alias || ""}](${this.value})`;
        break;
      case "External Link" /* ExternalLink */:
        output = `[${this.alias || ""}](${this.value})`;
        break;
      case "Text" /* Text */:
      default:
...
var SlotModal = class extends import_obsidian6.Modal {
  constructor(plugin, settings, header, value, onSubmit) {
    const app = plugin.app;
    super(app);
    this.settings = settings;
    this.plugin = plugin;
    ...
    let startAngle = 0;
    function isNearCorner(mouseX, mouseY, el) {
      if (!el)
        return false;
    ...
    this.renderEl.addEventListener("mousedown", (event) => {
      event.preventDefault();
    ...
    this.renderEl.addEventListener("wheel", (event) => {
      event.preventDefault();
      this.value.scale += event.deltaY * -5e-4;
    ...
    new import_obsidian6.Setting(flexSettingsEl).setName("Type").addButton((btn) => {
    ...
    new import_obsidian6.Setting(flexSettingsEl).setName("Content").addDropdown((dropdown) => {
      ...
      dropdown.onChange((value2) => {
        this.value.type = value2;
        this.plugin.settings.lastSlotType = value2;
        this.plugin.saveSettings();
        this.updateSettings();
        this.render();
      });
    });
    this.valueSetting = new import_obsidian6.Setting(flexSettingsEl);
    new import_obsidian6.Setting(flexSettingsEl).setName("Alias").addText((text) => {
      this.aliasSetting = text;
      text.setValue(this.value.alias || "").onChange((value2) => {
        this.value.alias = value2;
        this.render();
      });
    });
    new import_obsidian6.Setting(flexSettingsEl).setName("Color").addColorPicker((picker) => {
      this.colorPicker = picker;
      picker.setValue(this.value.color);
      picker.onChange((value2) => {
        this.value.color = value2;
        if (value2 != this.value.defaultColor) {
    ...
    new import_obsidian6.Setting(flexSettingsEl).setName("Transform").addButton((btn) => {
      this.mirrorXButton = btn;
      btn.setIcon("flip-horizontal").onClick(() => {
      ...
      btn.setIcon("flip-vertical").onClick(() => {
    ...
    new import_obsidian6.Setting(contentEl).addButton(
      ...
      (btn) => btn.setIcon("check").setCta().onClick(onSubmitHandler)
    );
    this.scope.register([], "Enter", (evt) => {
      evt.preventDefault();
      onSubmitHandler();
  ...
  updateSettings() {
    this.valueSetting.clear().setName("Value").addText((text) => {
      if (this.value.type == "Internal Embed" /* InternalEmbed */ || this.value.type == "Internal Link / Cover" /* InternalLink */) {
...
var SettingTab = class extends import_obsidian7.PluginSettingTab {
  constructor(app, plugin) {
    ...
    this.checkNumber = async (text, regex = /^(100|[1-9]?[0-9])$/) => {
  ...
  display() {
    ...
    new import_obsidian7.Setting(containerEl).setName("Number of slots").setDesc("How many slots will be displayed on one row before wrap to next line").addText((text) => {
    ...
    new import_obsidian7.Setting(containerEl).setName(`Use coloring`).addToggle((toggle) => {
    ...
    this.plugin.settings.tiers.forEach((tier, index2) => {
      const tierEl = tierListEl.createEl("div", { cls: "tier-item", attr: { "data-index": index2.toString() } });
      const tierLabel = tierEl.createEl("div", { text: `Tier \u2116${index2 + 1}`, cls: "setting-item-name" });
      const setting = new import_obsidian7.Setting(tierEl).addText(
        (text) => text.setPlaceholder("Name").setValue(tier.name).onChange(async (value) => {
          tier.name = value;
          await this.plugin.saveSettings();
        })
      );
      ...
      setting.addButton(
        (button) => button.setIcon("trash").setTooltip("Delete tier").onClick(async () => {
          this.plugin.settings.tiers.splice(index2, 1);
          await this.plugin.saveSettings();
          this.display();
        })
      );
    ...
    new import_obsidian7.Setting(containerEl).addButton(
      (btn) => btn.setButtonText("Add tier").onClick(async () => {
        this.plugin.settings.tiers.push({ name: "", color: "#ffffff" });
        await this.plugin.saveSettings();
        this.display();
      })
    );
    sortable_esm_default.create(tierListEl, {
      // handle: '.drag-handle',
      ...
      onEnd: async (evt) => {
        ...
        if (oldIndex2 !== newIndex2) {
          const movedItem = this.plugin.settings.tiers.splice(oldIndex2, 1)[0];
          this.plugin.settings.tiers.splice(newIndex2, 0, movedItem);
          await this.plugin.saveSettings();
          this.display();
        }
      }
...
// src/commands.ts
var insertTierListCommand = (settings) => {
  return {
    id: "tier-list-insert",
...
var constructTierList = (settings) => {
  let text = "";
...
// src/main.ts
var TierListPlugin = class extends import_obsidian8.Plugin {
  async onload() {

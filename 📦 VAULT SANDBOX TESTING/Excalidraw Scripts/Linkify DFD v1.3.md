...
```js*/
/*****************************************************************
 * Linkify DFD — v1.3  (2025-08-05)
 * ---------------------------------------------------------------
 * Added: Configurable transfer naming and property handling
 * Improved: Object-centric terminology and flexible separators
 ******************************************************************/
...
/* ---------- transfers with bidirectional support and improved properties ---------- */
async function ensureTransfer(arr) {
  ...
  if (!startId || !endId) {
    clog(`  ✗ Arrow not bound to both objects`);
    note("↯ arrow not bound to both objects");
    return;
  }
  ...
  if (!startEl || !endEl) {
    clog(`  ✗ Objects not found in element map`);
    note("↯ objects not found");
    return;
  }
  
  clog(`  ✓ Found objects: ${startEl.type} → ${endEl.type}`);
...
(async () => {
  ...
  clog("\n✅ Linkify DFD v1.3: finished");
  note("Linkify DFD v1.3: finished");
})();
